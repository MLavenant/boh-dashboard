...
console.log(`Total CN Lounge orders Jul 4: ${allOrders.length}`);

// Time filter: 11 PM - 5 AM crosses midnight
const byTable = {};
...
const sorted = Object.entries(byTable).sort(([,a],[,b])=>b-a);
console.log("\nRevenue by table (in BS window):");
for (const [name, amt] of sorted) console.log(`  ${name}: $${Math.round(amt)}`);
const total = sorted.reduce((s,[,a])=>s+a,0);
console.log(`\nTotal ALL tables in window: $${Math.round(total)}`);
console.log(`Expected: $28,522`);
