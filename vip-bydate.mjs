...
const TIERS={Diamond:["34","51","52"],Prestige:["31","41"],Platinum:["32","33","35","36","42","43","45","46","47","48","49","53","54","55","56"],Gold:["24","25","26","27","28"],Riverwalk:["19","20","21","22","23"],Cabana:["C1","C2","C3","C4","C5","C6","C7","C8","C9","C10","C1A","C2A","C3A","C4A","C5A","C6A","C7A","C8A","C9A","C10A"],Deck:["D1","D2","D3","D4","D5","D6","D7"]};
...
for(const date of dates){
  ...
  for(const o of allOrders){
    const tg=o.table?.guid??"";if(!bsGuids.has(tg))continue;
...
for(const date of dates){
  ...
  for(const[tier,tierTables]of Object.entries(TIERS)){
    ...
    result[date][tier]={soldTables:sold.length,totalTables:tierTables.length,totalSales:Math.round(totalSales*100)/100,avgPerTable:sold.length>0?Math.round(totalSales/sold.length*100)/100:0,minPerTable:MIN_PER_TABLE[tier]};
  }
}
