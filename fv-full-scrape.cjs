...
(async () => {
  ...
  for(const v of VENUES){
    ...
    console.log(v.name+": "+events.length+" events");
    ...
    for(const evt of events){
      ...
      if(mapData || kpiData){
        console.log("  ? "+evDate+" "+evt.nombre);
        if(kpiData) console.log("     KPIs:", JSON.stringify(kpiData.data||kpiData).slice(0,200));
        eventsData.push({ date:evDate, name:evt.nombre, id:evt._id, mapData, kpiData });
      } else {
        console.log("  ? "+evDate+" "+evt.nombre+" (no data)");
        eventsData.push({ date:evDate, name:evt.nombre, id:evt._id });
      }
    }
  ...
  console.log("\n? Done! Saved to fv-bookings-data.json");
  await browser.close();
})();
