...
async function calcForDate(date) {
  const allOrders = [];
  ...
  const startFrac = isSunday ? 0.75 : 0.958333; // 6PM for Sun, 11PM otherwise
  
  let total=0;
  for (const o of allOrders) {
    const isBs = bsGuids.has(o.table?.guid);
    const hasNoTable = !o.table?.guid;
    if (!isBs && !hasNoTable) continue;
    
    for (const c of (o.checks||[])) {
      if (c.voided) continue;
      const dt = new Date(new Date(o.openedDate).getTime()-4*60*60*1000);
      const frac = (dt.getUTCHours()*60+dt.getUTCMinutes())/1440;
      const inWindow = (frac>=startFrac || frac<=0.208333);
...
const tests = [
  ["2026-07-02", 28750.01, "Thu"],
  ["2026-07-03", 13130.01, "Fri"],
  ["2026-07-04", 28522, "Sat"],
  ["2026-07-05", 32187.50, "Sun"],
];
for (const [date, expected, day] of tests) {
  const actual = await calcForDate(date);
  const pct = Math.round(actual/expected*1000)/10;
  console.log(`${day} ${date}: $${actual} vs expected $${expected} (${pct}%)`);
}
