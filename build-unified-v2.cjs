...
body.printing-stations-pdf .ps-print-page.ps-intro {
  min-height:700px;
  display:flex;
...
body.printing-stations-pdf .ps-print-page.ps-intro .ps-intro-top {
  display:flex;
...
body.printing-stations-pdf .ps-print-page.ps-intro .ps-intro-top h1 {
  margin:0;
  font-size:26px;
}
body.printing-stations-pdf .ps-print-page.ps-intro .ps-sub {
  margin:0 0 12px;
  font-size:12px;
}
...
body.printing-stations-pdf .ps-print-page.ps-intro .ps-chart-wrap {
  flex:1 1 auto;
  display:flex;
  align-items:center;
...
body.printing-stations-pdf .ps-print-page.ps-intro img.ps-chart {
  width:100%;
  max-height:340px;
  object-fit:contain;
  background:#13161c;
...
body.printing-stations-pdf .ps-print-page.ps-intro .ps-matrix-wrap {
  margin-top:auto;
}
...
body.printing-stations-pdf .ps-print-page.ps-intro .ps-rdg-logo .ps-rdg-letters {
  font-family:Arial,Helvetica,sans-serif;
  font-size:18px;
  font-weight:300;
  letter-spacing:0.2em;
...
body.printing-stations-pdf .ps-print-page.ps-intro .ps-rdg-logo .ps-rdg-sub {
  font-size:7px;
...
function buildPortfolioStationsFamilyTableHtml(family, weekKey, opts) {
  opts = opts || {};
  const labels = ${JSON.stringify(VENUE_LABELS)};
  const venueKeys = Array.isArray(opts.venueKeys) && opts.venueKeys.length
    ? opts.venueKeys
    : null;
  const venues = venueKeys
    ? venueKeys.map(k => ({ key: k, label: labels[k] || k })).filter(v => {
        const sc = buildVenueWeekScorecard(v.key, v.label, weekKey);
        const st = sc.familyStats[family] || {};
        if (st.ipsh != null && st.ipsh > 0) return true;
        if (st.iph != null && st.iph > 0) return true;
        ...
        return HOURLY_DAYS.some(day => {
          const m = getFamilyDayMetrics(v.key, weekKey, family, day);
          return m && ((m.heads > 0 && m.items > 0) || (m.items > 0));
        });
      })
    : portfolioVenuesForFamily(family, weekKey);
  // Placeholder / forced single-venue: still render shell even if empty
  const forceVenues = Array.isArray(opts.forceVenueKeys) && opts.forceVenueKeys.length
    ? opts.forceVenueKeys.map(k => ({ key: k, label: labels[k] || k }))
    : null;
  const useVenues = (venues.length ? venues : (forceVenues || []));
  ...
  const weekCells = useVenues.map(v => {
    const sc = buildVenueWeekScorecard(v.key, v.label, weekKey);
    const st = sc.familyStats[family] || {};
...
function buildStationsPdfIntroHtml(weekLabel, chartImg, families, venueRows, versionTag) {
  let intro = '<div class="ps-print-page ps-intro">'+
    '<div class="ps-intro-top">'+
      '<div><h1>RDG Portfolio - Stations</h1>'+
      '<p class="ps-sub" style="margin:6px 0 0">'+weekLabel+' | '+versionTag+'</p></div>'+
      rdgLogoHtml('light')+
    '</div>';
  if (chartImg) intro += '<div class="ps-chart-wrap"><img class="ps-chart" src="'+chartImg+'" alt="Stations compare chart"></div>';
  intro += '<div class="ps-matrix-wrap"><h2>Week matrix — items / staff-hour</h2><table><thead><tr><th>Station</th>';
  ...
  intro += '</tbody></table></div></div>';
  return intro;
}

function buildStationsPdfPortfolioPagesHtml(families, weekKey, weekLabel) {
  const FAMILIES_PER_PAGE = 3;
  return chunkArray(families, FAMILIES_PER_PAGE).map((chunk, idx) => {
...
function ensurePortfolioStationsContext() {
  if (currentVenue !== 'rdg_portfolio') {
    currentVenue = 'rdg_portfolio';
    document.querySelectorAll('.venue-pill').forEach(b => {
      b.classList.toggle('active', b.dataset.venue === 'rdg_portfolio');
...
finalHtml = finalHtml
  ...
  .replace(
    ...
    '<div class="annotation-box" id="bpAnnotation">⚡ Breaking point at <strong>26 concurrent tickets</strong> — avg fulfillment jumps to 16.0 min.</div><div id="bpMethodNote" style="font-size:11px;color:#9aa0aa;margin-top:4px">BP = first load where avg fulfillment crosses target</div>'
  );
