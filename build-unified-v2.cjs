...
// Add Lunch vs Dinner service split card after heatmap card (before closing section)
html = html.replace(
  '      <span><span class="sw" style="background:#1a1d24"></span>No data</span>\n    </div>\n  </div>\n</div>\n\n</section>\n\n<!-- ========== TAB 2: STATIONS ========== -->',
  `      <span><span class="sw" style="background:#1a1d24"></span>No data</span>\n    </div>\n  </div>\n</div>\n\n<div class="card" id="serviceSplitCard">
  <h2>🌞 Lunch vs 🌙 Dinner — Service Comparison</h2>
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:16px">
    <div style="background:#1a1e28;border-radius:8px;padding:16px;border-left:3px solid #f59e0b">
      <div style="color:#f59e0b;font-weight:700;margin-bottom:10px">🌞 LUNCH (11am–3pm)</div>
      <div class="kpis">
        <div class="kpi"><div class="v" id="ssLunchFul">—</div><div class="l">Avg fulfillment</div></div>
        <div class="kpi"><div class="v" id="ssLunchGuests">—</div><div class="l">Total guests</div></div>
        <div class="kpi"><div class="v" id="ssLunchPeak">—</div><div class="l">Peak hour</div></div>
      </div>
    </div>
    <div style="background:#1a1e28;border-radius:8px;padding:16px;border-left:3px solid #ef4444">
      <div style="color:#ef4444;font-weight:700;margin-bottom:10px">🌙 DINNER (5pm–11pm)</div>
      <div class="kpis">
        <div class="kpi"><div class="v" id="ssDinnerFul">—</div><div class="l">Avg fulfillment</div></div>
        <div class="kpi"><div class="v" id="ssDinnerGuests">—</div><div class="l">Total guests</div></div>
        <div class="kpi"><div class="v" id="ssDinnerPeak">—</div><div class="l">Peak hour</div></div>
      </div>
    </div>
  </div>
  <canvas id="lunchDinnerChart" height="120"></canvas>
</div>\n\n</section>\n\n<!-- ========== TAB 2: STATIONS ========== -->`
);
...
function computeBreakingPoint() {
  ...
  for (let i = 0; i < curve.length; i++) {
    // Skip first 10 load levels (index 0-9, i.e. 1-10 concurrent tickets)
    if (i < 10) continue;
    const d = curve[i];
    if (d.occ < 5) continue;
  ...
  function renderStationDetail(s) {
    ...
    document.getElementById('stationDetail').innerHTML =
      ...
      (items.length > 0 ? '<table style="width:100%;border-collapse:collapse;font-size:12px"><thead><tr><th style="text-align:left;color:#9aa0aa;padding:4px 8px">Item</th><th style="text-align:right;color:#9aa0aa;padding:4px 8px">Avg Time</th><th style="text-align:right;color:#9aa0aa;padding:4px 8px">Tickets</th></tr></thead><tbody>' +
        items.slice(0,3).map(it => '<tr><td style="padding:4px 8px;color:#e8eaed">' + it.item + '</td><td style="padding:4px 8px;text-align:right;font-weight:600;color:' + (it.avg_sec > (s.exp_sec||900) ? '#ef4444' : '#22c55e') + '">' + fmtSec(it.avg_sec) + '</td><td style="padding:4px 8px;text-align:right;color:#9aa0aa">' + it.count + '</td></tr>').join('') +
        '</tbody></table>'
...
function renderMenuItems() {
  ...
  Object.entries(STATION_ITEMS).forEach(([station, items]) => {
    ...
    (items || []).forEach(it => {
      if (!itemStationMap[it.item]) itemStationMap[it.item] = station;
    });
  });
  ...
  Object.entries(STATION_ITEMS).forEach(([station, items]) => {
    ...
    (items || []).forEach(it => {
      // Use item's own exp_sec if available (from summary), else use station target
      if (!itemExpSecMap[it.item] && stnExpSec > 0) {
        itemExpSecMap[it.item] = stnExpSec;
      }
    });
  });
...
// ============================================================
// VISUAL: Lunch vs Dinner Service Split
// ============================================================
function renderServiceSplit() {
  const HMF = getD().hmFul || {};
  const HMG = getD().hmGuests || {};
  const DAYS = ['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday'];
  const LUNCH_HRS = ['11','12','13','14'];
  const DINNER_HRS = ['17','18','19','20','21','22'];

  function serviceStats(hrs) {
    let fulSum=0, fulCount=0, guestSum=0;
    let peakHr='—', peakVal=0;
    DAYS.forEach(day => {
      hrs.forEach(hr => {
        const f = HMF[day]&&HMF[day][hr]!=null ? HMF[day][hr] : null;
        const g = HMG[day]&&HMG[day][hr] ? HMG[day][hr] : 0;
        if (f!=null) { fulSum+=f; fulCount++; if(f>peakVal){peakVal=f;peakHr=hr+':00';} }
        guestSum += g;
      });
    });
    return { ful: fulCount>0?(fulSum/fulCount).toFixed(1)+'min':'—', guests: guestSum, peak: peakHr };
  }

  const lunch = serviceStats(LUNCH_HRS);
  const dinner = serviceStats(DINNER_HRS);

  const set = (id,v) => { const e=document.getElementById(id); if(e) e.textContent=v; };
  set('ssLunchFul', lunch.ful); set('ssLunchGuests', lunch.guests); set('ssLunchPeak', lunch.peak);
  set('ssDinnerFul', dinner.ful); set('ssDinnerGuests', dinner.guests); set('ssDinnerPeak', dinner.peak);

  const days = ['Mon','Tue','Wed','Thu','Fri','Sat','Sun'];
  const fullDays = ['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday'];
  const lunchData = fullDays.map(day => {
    const vals = LUNCH_HRS.map(hr => HMF[day]&&HMF[day][hr]!=null?HMF[day][hr]:null).filter(v=>v!=null);
    return vals.length ? vals.reduce((a,b)=>a+b,0)/vals.length : null;
  });
  const dinnerData = fullDays.map(day => {
    const vals = DINNER_HRS.map(hr => HMF[day]&&HMF[day][hr]!=null?HMF[day][hr]:null).filter(v=>v!=null);
    return vals.length ? vals.reduce((a,b)=>a+b,0)/vals.length : null;
  });

  const thrLine = {id:'thr2',afterDraw(chart){const{ctx,chartArea:a,scales}=chart;if(!a||!scales.y)return;const yy=scales.y.getPixelForValue(15);if(yy<a.top||yy>a.bottom)return;ctx.save();ctx.strokeStyle='#e2706a';ctx.lineWidth=1.5;ctx.setLineDash([6,4]);ctx.beginPath();ctx.moveTo(a.left,yy);ctx.lineTo(a.right,yy);ctx.stroke();ctx.setLineDash([]);ctx.fillStyle='#e2706a';ctx.font='11px sans-serif';ctx.fillText('15 min target',a.left+6,yy-4);ctx.restore();}};

  const el = document.getElementById('lunchDinnerChart');
  if (el) {
    if (el._chart) el._chart.destroy();
    el._chart = new Chart(el, {
      type:'bar',
      data:{ labels:days, datasets:[
        {label:'🌞 Lunch',data:lunchData,backgroundColor:'rgba(245,158,11,0.7)',borderRadius:4},
        {label:'🌙 Dinner',data:dinnerData,backgroundColor:'rgba(239,68,68,0.7)',borderRadius:4}
      ]},
      options:{plugins:{legend:{labels:{color:'#9aa0aa'}}},scales:{
        x:{grid:{display:false},ticks:{color:'#9aa0aa'}},
        y:{title:{display:true,text:'Avg fulfillment (min)'},grid:{color:'#262a33'},ticks:{color:'#9aa0aa'},suggestedMax:20}
      }},
      plugins:[thrLine]
    });
  }
